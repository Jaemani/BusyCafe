...
서울의 현재·평소 대비 **도시 활동도**를 근거와 함께 지도에서 보여주고, 카페를 첫 번째
탐색 레이어로 제공하는 서비스입니다. 현재 공개 프리뷰의 카페 마커는 매장 좌석 점유율이
아니라 **카페 주변 지역의 혼잡도 추정치**입니다.

**공개 프리뷰:** https://busy-cafe.vercel.app

> 현재 공개 프리뷰는 Supabase PostgreSQL을 읽습니다. 다만 GitHub Actions worker에서
> 서울 API 연결 지연이 반복되어 자동 수집은 안전하게 중지한 상태입니다. 화면은 마지막
> 완전 수집 시각과 갱신 지연을 표시합니다. 별도 상시 worker가 운영되기 전에는
> 실시간 서비스로 간주하지 않습니다.

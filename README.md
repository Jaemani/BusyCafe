...
서울시 실시간 도시데이터를 카페 위치에 연결해 **카페 주변 지역의 현재 혼잡도 추정**을
...
> 서울 원천 자체의 관측 지연은 별도이므로 화면의 관측 시각과 `지연 참고용` 표시를
> 함께 확인해야 합니다.

## 현재 제공하는 기능

- MapLibre/OpenFreeMap 기반 서울 지도와 카페 클러스터
- 19개 지하철 노선, 380개 역 지점과 2,126개 출구를 보여주는 선택적 지도 맥락
...
- 카페별 주변 혼잡도, coverage, 관측 지역·거리·시각과 근거 강도를 묶은 간결한 상세
- 노치와 홈 영역의 안전 여백을 반영한 모바일 화면과 간결하게 접고 펼치는 카페 상세
- 현재 방문에서 닫을 수 있는 출퇴근시간 안내
- 검증 대기열로만 들어가는 장소 신고와 주변 혼잡·매장 좌석 분리 피드백
- 지도 영역 조회 API와 상태 확인 API

현재 근거 강도는 실제 적중 확률이 아니라 **입력 근거의 충분도**입니다. 지역 보행 혼잡과
카페 좌석 여유의 상관은 아직 현장 관측으로 검증되지 않았습니다.

장기 제품 코어는 폴리곤·250m 셀 단위 도시 활동도 surface이며 카페는 그 위의 첫
overlay입니다. 관측, 예측, 평소 패턴과 stale 상태를 분리하고, activity shadow가 현장
검증 gate를 통과하기 전에는 현재 카페 중심 API/UI를 유지합니다.
...
프리뷰는 이 구조의 Supabase 읽기 경로를 사용하며, 외부 수집 장애 때도 마지막 완전
cycle을 보존하고 stale 상태를 표시합니다.

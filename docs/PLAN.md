...
> **v1.9 (2026-07-13)**: 제품 코어를 카페 혼잡 추정이 아니라 상대적 도시 활동도
...
2. **절대값이 아니라 자기 평소 대비 활동도.** 서로 다른 source·도시의 원시 인구와
   통행량을 직접 비교하지 않는다.
...
- calibration 전 도시 간 절대 활동도 순위 또는 정확도 확률 제공
...
- 코어 연구 산출물은 250m cell/polygon 단위 도시 활동도 surface다.
- 카페 마커는 surface를 소비하는 첫 overlay이며 내부 혼잡으로 설명하지 않는다.
- 현재 공개 MVP의 카페 중심 API/UI는 호환성을 위해 유지한다.
- activity surface는 offline shadow에서 먼저 검증하고 Phase 6 통과 전 기본 레이어나 공개
  문구로 승격하지 않는다.
...
| 서울 250m 생활인구 | 요일·시간·공휴일별 도시 활동도 베이스라인 | 시간별 (약 4일 지연) | 월별 파일/API → offline Parquet | Track 1 shadow |

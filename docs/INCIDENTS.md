...
## INC-2026-019 — 생활인구 CELL_ID·숫자 문법 가정 및 CP949 native crash

- 상태: Monitoring (producer/consumer 해결, 월 30파일 전수 검증 대기)
- 심각도: SEV-3 (공개·production 영향 없음)
- 시작/감지: 2026-07-15
- 작성자: Codex
...
당시 기존 compactor는 date-hour-cell 중복을 거부했고 cell 단위 Parquet에서 행정동 부분행을
어떻게 합칠지 정하지 못했다. 일부 부분행이 `*`일 때 known 합계와 masked fragment 수를 함께
보존해야 하므로 단순 합산이나 전체 cell 마스킹으로 임의 우회하지 않고 v2 계약을 먼저
확정했다.

후속 v2 producer는 원천 4-key를 보존하면서 `(date,hour,CELL_ID)`별 `known_total`,
`fragment_count`, `masked_fragment_count`, 결정적 `fragments_json`을 만든다. 숫자 fragment만
정확한 `DECIMAL(38,5)` 합계에 포함하고 부분 마스킹을 cell 전체 마스크로 접지 않는다.
consumer는 sidecar manifest의 schema/query version·파일명·크기·SHA-256·행 수와 fragment
불변식을 검증하며, 부분 또는 전체 마스킹 target에는 현재값을 점·구간 대치하지 않고
baseline-only 또는 unsupported로 abstain한다.

`2026-06-30` 실 CSV apply 중 DuckDB의 CP949 decoder가 SIGSEGV(exit 139)를 냈다. crash는
Parquet COPY 전 source bind/scan 경로에서 발생했고 artifact, 공개 output, `.part` 파일은
남지 않았다. Python strict streaming parser가 CP949 원본 전체를 검증하며 필요한 6열 UTF-8
임시 CSV를 만들고 DuckDB는 UTF-8만 읽도록 바꿨다. 정상·오류 경로 모두 임시 디렉터리를
정리하는 회귀 테스트를 추가했고 같은 실파일 apply를 두 번 통과시켜 Parquet SHA 일치를
확인했다. 다만 2026-06 월 30파일 전수 검증은 끝나지 않아 상태는 Monitoring을 유지한다.
...
- [x] compactor v2에 `known_total + masked_fragment_count + fragments_json` 원본 의미를
  보존하는 schema 확정
- [x] activity artifact/baseline consumer가 부분 마스킹을 임의 점·구간 대치하지 않고
  abstain하는 계약과 manifest 검증 추가
- [x] DuckDB CP949 decoder를 strict Python parser→UTF-8 temp 경로로 격리하고 cleanup 검증
- [ ] 전체 2026-06 월 30파일 compaction dry-run/apply 재검증
...
## INC-2026-021 — 동일 인허가 원장을 독립 capacity 근거로 자기매칭

- 상태: Resolved before release
- 심각도: SEV-3 (공개·production 점수 영향 없음)
- 시작/감지/해결: 2026-07-15
- 작성자: Codex
- 관련 커밋: `8840ac3`, `3ab9241`

### 요약

첫 production capacity coverage report에서 4,637건이 strict entity match로 보고됐다.
그러나 verified 카페의 `origin_provider`를 집계하니 모두
`seoul_refreshment_permits`였다. 즉 인허가에서 파생한 카페 원장을 같은 인허가로
다시 검증한 결과였으며 독립 근거가 아니었다. 해당 report를 폐기했고 공개
점수·API·DB에는 반영하지 않았다.

### 근본 원인과 대응

인허가 후보와 모든 active cafe를 비교하면서 source 독립성을 spatial index 구성
전에 검증하지 않았다. 이후 동일 source를 후보에서 전체 제외하고, 전체·독립
카페·제외 수를 report에 명시하며, same-source 행이 거리·모호성·성공 집계에도
참여하지 못하게 회귀 테스트를 추가했다.

### 재발 방지

- [x] 후보 index 구성 전 `origin_provider` 독립성 강제
- [x] 제외 source와 제외 수를 집계 report에 보존
- [x] same-source가 성공·모호성·단계별 진단에 영향을 주지 못하는 회귀 테스트
- [x] 기존 4,637건 report 폐기와 public non-impact 기록
- [ ] 독립 source 매칭의 HUMAN precision sample 통과

### 교훈

스키마가 다른 두 테이블을 매칭했다고 독립 근거가 되지는 않는다. 파생 lineage를
먼저 차단하고 나서야 거리·주소·이름 일치를 독립 검증 근거로 해석할 수 있다.

## INC-2026-022 — Production 24시간 ingest complete SLO 미달

- 상태: Monitoring
- 심각도: SEV-3 (수집 가용성·신선도 저하, stale 현재값 위장은 차단됨)
- 시작/감지: 2026-07-14~15 / 2026-07-15
- 작성자: Codex
- 관련 커밋: `a542e4a`

### 요약

Production 24시간 289개 terminal cycle 중 complete 273, failed 14, partial 2로 complete
rate는 94.46%였다. Target 저장 성공률도 95.15%로 운영 목표 99%에 미달했다.
121개 hotspot은 모두 window 내에 관측됐지만 source observation lag는 p50 32.66분,
p95 32.88분이었다. 공개 API의 delayed/stale masking이 오래된 값을 fresh로 표시하는
것은 막았으나, 수집 연속성과 25분 신선도 목표는 통과하지 못했다.

### 근본 원인과 대응

14개 full failure에서는 첫 5개 hotspot이 각각 4회 `ConnectTimeout`을 낸 뒤 circuit가
열렸다. 2개 partial은 no-record/parse 응답으로 120/121, 119/121개를 저장했다.
DB commit이나 materialize 실패 증거는 없었다. GitHub eastus runner에서 서울 API로
연결하는 경로의 일시적 문제로 보이지만 지역·원인은 확정하지 않았다. 분석기 schema
v2가 비정상 terminal cycle별 시각·지속시간·저장 수·신호를 노출하게 해 원인
분리 증거를 남겼다.

### 재발 방지

- [x] failed/partial cycle별 시각·저장 수·신호를 결정적 report에 추가
- [x] 24시간·6시간 read-only SLO 재측정
- [x] stale/delayed 안전장치와 public health 정상 작동 확인
- [ ] 운영 시작 후 연속 7일 전체 window SLO 재측정
- [ ] timeout 발생 지점과 서울 API 응답 상태를 연결 경로별로 분리
- [ ] 7일 complete rate 99% 미달 시 서울 리전 상시 worker 검토
- [x] 원인 확정 전 concurrency·retry 증가 금지

### 교훈

Coverage 100%는 주기적 수집 99%를 의미하지 않는다. 날창 누적 coverage, cycle complete
rate, 대상별 성공률, source observation lag를 분리해야 사용자가 보는 신선도를
정확히 운영할 수 있다.

## INC-2026-023 — Ready deployment와 canonical Vercel alias 불일치

- 상태: Mitigated, recurrence confirmed
- 심각도: SEV-3 (기존 서비스는 가용했으나 최신 검색 기능이 공개 URL에 반영되지 않음)
- 시작/감지/해결: 2026-07-15
- 작성자: Codex
- 관련 커밋: `1048c16`, `e283225`

### 요약

GitHub CI와 Vercel production deployment가 성공했지만 `busy-cafe.vercel.app`은 약 7시간 전
deployment를 계속 가리켰다. 최신 preview deployment에서는 새 JavaScript asset과 거리순
검색 API가 동작했으나 canonical domain은 이전 asset과 전역 relevance 검색 결과를 반환했다.
Vercel CLI로 canonical alias를 최신 Ready deployment
`dpl_FtGiguYPeRtUwQP5Uhca4nFBmQKZ`에 다시 연결한 뒤 동일 요청으로 복구를 확인했다.
다음 기능 배포 `71d98bf`에서도 최신 deployment가 Ready인 동안 canonical domain은 이전
`index-Cl0qUWFv.js`를 계속 반환해 같은 현상이 재현됐다. Preview에서 새 브랜드 검색과
`index-Bzy8Gu66.js`를 확인한 뒤 canonical alias를
`dpl_3xaKKFMomPMdS2EJf2T3grvCXDX4`로 다시 연결했다.

### 근본 원인과 대응

새 deployment는 `target=production`, `status=Ready`였고 GitHub Vercel status도 success였지만,
canonical alias가 자동 승격되지 않았다. 자동 alias 갱신이 멈춘 이유는 아직 확인하지 못했다.
따라서 Vercel status success를 공개 승격 증거로 사용하지 않고 canonical domain의 asset과
대표 API 응답을 별도로 확인했다. 시청 기준 스타벅스 검색은 복구 뒤 무교로점, 시청점,
한국프레스센터점 순으로 반환됐다.

### 재발 방지

- [x] 최신 deployment URL과 canonical domain의 asset/API를 분리 확인
- [x] canonical alias를 최신 Ready production deployment로 재연결
- [x] 후속 기능 배포에서 동일 현상 재현 및 수동 승격 절차 재검증
- [x] 배포 runbook에 canonical alias smoke를 필수 승격 단계로 구체화
- [ ] `/api/health`에 non-secret deployment revision을 노출해 기대 commit과 자동 비교
- [ ] CI 성공과 독립된 canonical domain post-deploy check 및 알림 추가
- [ ] Vercel production alias 자동 승격이 중단된 원인 확인

### 교훈

빌드 성공과 deployment Ready는 사용자가 접속하는 도메인의 승격을 보장하지 않는다. 공개
검증은 preview URL이 아니라 canonical domain에서 실행하고, 코드 revision 또는 asset
fingerprint까지 비교해야 한다.

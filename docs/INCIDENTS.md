...
배포 전 프로젝트명과 최종 공개 URL을 확인하는 단계가 없었고, 프로젝트 rename 뒤 alias
연결과 공개 접근 정책을 하나의 검증 단위로 다루지 않았다. Vercel 프로젝트를
`busy-cafe`로 rename하고 새 production deployment를 만든 뒤 alias를 다시 연결했다.
...
- [ ] 배포 체크리스트에 프로젝트명·alias·SSO 정책·공개 endpoint 검증 추가

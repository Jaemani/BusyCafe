...
- 서울시 역사마스터와 버전을 고정한 OpenStreetMap 노선·출구 원본으로 만든 서울 지하철
  오버레이. 19개 노선을 고유 색상으로 표시하고, 역명은 확대 수준에 따라 단계적으로
  노출한다. 출구 번호는 zoom 15.5부터 검정 테두리의 노란 배지로 표시한다. 세 정적
  GeoJSON은 CDN 캐시하며 일부 원본이 실패해도 카페 지도는 계속 동작한다
  (`0185e8b`, `c9087bc`, `e600cdd`)
- 저장된 서울 관측을 `t+10·30·60·90·120분`의 실제 후속 관측과 짝지어 지연에 따른 정보
  열화를 측정하는 read-only source persistence shadow. 원시 인구 상관은 같은 장소의
  시간축에서만 계산하며 공개 점수·신선도 정책·API에는 영향을 주지 않는다(`d3d8df2`)
...
- 모바일 viewport가 `viewport-fit=cover`와 동적 viewport 높이를 사용하고, 상단 검색·안내,
  카페 상세, 범례와 지도 control의 네 방향 여백에 safe-area inset을 반영하도록 변경했다.
  모바일 상세는 노치 아래에서 최대 높이를 제한하고 내부 스크롤을 유지한다(`8e38aa8`)
- 지도 상단의 서비스 안내·개인정보 링크와 현재 상태 metadata를 한 줄 안에서 간결하게
  배치하고, 서비스 안내는 `/about.html#recent-updates`로 바로 연결한다(`213b2a7`)
...
- 카페 상세의 release timestamp·숫자형 원장 품질·중복 `참고용` 문구를 사용자용 표현으로
  축약. 혼잡도, 관측 지점·거리, 경계 여부와 데이터 나이를 각각 한 번만 표시한다. 모바일
  하단 시트는 핵심 정보를 먼저 보여주고 위쪽 손잡이로 전체 내용을 펼치거나 다시 접는다.
  주소·전화번호·웹사이트는 한 줄에 배치하고 사용자에게 필요하지 않은 장소 원장 출처 줄은
  숨긴다. 외부 지도 링크 옆의 `피드백 주기` 버튼을 한 번 누르면 시트와 양식이 함께 열리고
  첫 선택 항목에 focus한다. 원본 metadata와 상세 라이선스는 API와 `/about.html`에
  보존한다(`de617fa`, `101dcfa`, `6575cb5`, `fed7ac2`, `bc9054e`)
...
- 모바일 검색창과 누락 카페 입력의 computed font size를 16px로 고정해 iPhone Safari가
  ...
  계속 허용한다
- 프랜차이즈 브랜드 칩은 기본으로 숨기고 상단 카드 우하단의 `브랜드` 버튼으로 열도록
  바꿨다. 버튼은 카드 접기 옆에 두고, 카드를 접으면 열린 브랜드 목록도 닫는다. 가로 스크롤
  영역의 아래 padding을 보강해 브랜드 칩 테두리가 살짝 잘리던 문제도 교정했다
...
- iOS Safari의 브라우저 주소창이 만드는 실제 `VisualViewport`와 고정 앱 좌표계가 어긋나
  ...
  grid의 intrinsic 최소 폭이 402px 화면에서 604.5px까지 늘어나던 원인을 막아 검색 버튼과
  오른쪽 안내가 화면 밖으로 밀리지 않게 했다(`b7f3d30`, `f83da42`, `a9cece4`)
- `body`의 20rem 최소 폭과 `100vw` 기반 폭 계산이 좁은 viewport를 가로로 넘길 수 있던
  모바일 레이아웃. 최소 폭과 `100vw` 계산을 제거하고 앱은 고정 inset, 지도는 그 안의 절대
  inset으로 두며 상단 shell을 좌우 safe-area edge 사이의 자동 폭으로 바꿨다. 기본 상세
  패널에도 `vh` fallback과 `dvh` 최대 높이·세로 스크롤을 적용해 짧은 가로 화면을 포함하고,
  모바일 시트는 양쪽 edge 사이의 자동 폭을 사용한다. safe-area 간격은 기본 여백에 inset을
  더하지 않고 `max(기본 여백, inset)`을 사용해 화면이 과도하게 줄어들지 않게 했다. 이미
  오른쪽 inset 안에 있는 기본 패널의 닫기 버튼은 여백을 다시 적용하지 않고, edge-to-edge
  모바일 시트에서만 오른쪽 inset을 적용한다(`5c572b8`, `aca0ff3`, `2efcaa7`). 후속 iOS
  Safari 계측에서 safe-area 값보다 grid 최소 폭과 VisualViewport 분리가 직접 원인이었음을
  확인해 위 수정으로 교정했다. 물리 Galaxy 검증은 아직 완료하지 않았다
- 카페 상세를 연 직후 지도 요약 갱신이 엇갈리면 불완전하거나 더 오래된 요약이 최신 상세의
  카페명·관측 지역·거리·시각을 덮어쓸 수 있던 문제. 불완전 요약은 현재 상태만 갱신하면서
  완전한 상세 근거를 보존하고, 관측 시각이 더 오래된 요약은 무시한다(`101dcfa`)

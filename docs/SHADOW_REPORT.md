...
판정 규칙은 [`PHASE6_PREREGISTRATION.md`](PHASE6_PREREGISTRATION.md)에 사전 등록했다.
아래 요약이 그 문서와 충돌하면 사전 등록 문서가 우선한다.

1. 기존 Phase 6 primary 기준선(S1)은 그대로 수집한다. S1은 v1/v2 비교에 쓰지 않는다.
2. 상위 divergence 목록 중 level 변경 20곳(S2)을 사전 등록된 sign test로 paired
   관측한다. v1/v2 우열 판정은 S2에서만 나온다.
3. v2가 낮게 예측한 관측에서 2레벨 이상 과소 추정이 1건이라도 나오면 승격 불가
   (비대칭 guardrail).
4. coverage loss 0은 품질 기준이 아니라 제품 연속성 제약이다. 스코어 모델 교체와
   coverage 확장 활성화는 분리 결정한다(확장은 S3 report + HUMAN 리뷰 후).
